...
test5 = TestCase $ assertEqual "Pawn tries to move straight at an enemy piece" False (isValidMove  testBoard2 (Piece Pawn (7,1) Black) (6,1))
--test for possibleMoves for a queen
test6 = TestCase $ assertEqual "all possible moves for queen from (3,3) " [(3,4),(3,5),(3,6),(3,7),(3,8),(4,4),(5,5),(6,6),(7,7),(8,8),(4,3),(5,3),(6,3),(7,3),(8,3),(4,2),(5,1),(3,2),(3,1),(2,2),(1,1),(2,3),(1,3),(2,4),(1,5)] (possibleMoves (Piece Queen (3,3) White))

--test for isCheckMate
test7 = TestCase $ assertEqual "checkMate test" False (isCheckMate (Piece King (6,4) Black) checkMateTestBoard)

--tests for isStaleMate
test8 = TestCase $ assertEqual "stalemate test when true" True (isStaleMate staleMateTestBoard1 (Piece King (1,1) White))
test9 = TestCase $ assertEqual "stalemate test when false" False (isStaleMate staleMateTestBoard2 (Piece King (1,1) White))

--test for isValidMove by trying to move king into a check position
test10 = TestCase $ assertEqual "king move to check position" False (isValidMove staleMateTestBoard2 (Piece King (1,1) White) (2,1))

-- test for isQueening 
test11 = TestCase $ assertEqual "check if queening is True" True (isQueening (Piece Pawn (2,7) Black) (1,7))

--tests for checking isValidMove on moves at a game of school mate
...
-- test for isCheckMate at a school mate board
testSchool8 = TestCase $ assertEqual "isCheckMate at school mate board" True (isCheckMate (Piece King (8,4) Black) schoolBoard7)

runtest = runTestTT $ TestList [test1,test2,test3,test4,test5,test6,test7,test8,test9,test10,test11]
